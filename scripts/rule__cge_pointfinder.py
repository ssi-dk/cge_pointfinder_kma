...
def rule__run_cge_pointfinder(input, output, organism, sampleComponentObj, log):
    try:
        ...
        # Code to run
        sampleComponentObj.rule_run_cmd("kma index -i /bifrost/pointfinder/pointfinder_db/{}".format(
            input.contigs, output, organism), log)
        sampleComponentObj.rule_run_cmd("/bifrost_resources/pointfinder/PointFinder.py -i {} -o {} -s {} -p /bifrost/pointfinder/pointfinder_db -m kma -m_p /opt/conda/pkgs/blast-2.9.0-h20b68b9_1/bin/blastn".format(
            input.contigs, output, organism), log)
